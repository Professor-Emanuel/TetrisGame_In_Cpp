...
Board::Board(Vec2<int> screenPos, Vec2<int> widthHeight, int cellsize_in, int padding)
	: screenPos(screenPos), width(widthHeight.GetX()), height(widthHeight.GetY()), cellSize(cellsize_in), padding(padding)
{
...
{
	raycpp::DrawRectangleLinesEx(screenPos - (cellSize / 2), Vec2{ width * cellSize, height * cellSize } + cellSize, cellSize/2, WHITE);
}
...
void Board::Draw() const
{
	for (int iY = 0; iY < height; ++iY) {
		for (int iX = 0; iX < width; ++iX) {
			if (CellExists({ iX, iY })) {
				DrawCell({ iX, iY }); // same as DrawCell(Vec2<int>{ iX, iY });
			}
		}
	}
